    ...
    <section class="create-services-screen">
        <div class="row create-services-screen-left">
            <div class="for-our-services">
                <h3>City Charges</h3>
                @can('charges-create')
                    <a href="{{ route('charges.create') }}"><svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                ...
                <tbody>
                    @foreach ($charges as $key => $charge)
                        <tr>
                            ...
                            {{-- @if ($charge->status == '1')
                            <td>Paid</td>
                                <td><a class="btn btn-success" href="{{ route('charges.pay', $charge->id) }}">Paid</a></td>
                            @elseif ($charge->status == '0')
                            <td>UnPaid</td>
                                <td><a class="btn btn-danger" href="{{ route('charges.pay', $charge->id) }}">Pay</a></td>
                            @else
                            <td>Disputed</td>
                                <td><a class="btn btn-primary" href="{{ route('charges.pay', $charge->id) }}">Disputed</a>
                                </td>
                            @endif --}}
                            @can('charges-edit')
                                <td>
                                    <a href="{{ route('charges.edit', $charge->id) }}" class="btn btn-success">Update</a>
                                </td>
                            @endcan
                            @can('charges-delete')
                                <td>
